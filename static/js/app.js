    ...
    function ($locale) {
        angular.extend($locale, {
            ...
            FILTER: {
                ...
                orderStatus:['下单','等待处理','未付款','已付款，待联系','联系不上买家','蛋糕制作中','订单取消，待退款','处理退款中','配送中，待确认','交易成功',"交易成功（含退款）","交易取消"],
...
jsGen
    ...
    .controller('orderCakeCtrl', ['app', '$scope',
        function (app, $scope) {
            var global = app.rootScope.global,
            ...
            $scope.selectEight();
                ...
                if((global.order.shopperId && (global.order.shopperId === global.user._id)) ){
                    app.toast.success("用户不能购买自己发布的商品！");
                    $scope.nextDisable = true;
                }
                if(global.selectCake && (global.selectCake.author._id === global.user._id)){
                    app.toast.success("用户不能购买自己发布的商品！");
                    $scope.nextDisable = true;
                }
            ...
            $scope.makeOrder = function () {
                if((global.order.shopperId && (global.order.shopperId === global.user._id)) ){
                    app.toast.success("用户不能购买自己发布的商品！");
                }else if(global.selectCake && (global.selectCake.author._id === global.user._id)){
                    app.toast.success("用户不能购买自己发布的商品！");
                }else{
                    var data = {
                    ...
                    if (app.validate($scope)) {
                        //更新用户信息
                        //更新地址信息
                        if(global.user.desc==="" || global.user.phoneNumber===""){
                        app.restAPI.user.save({
                        }, {
                            desc: global.user.desc===""?$scope.desc:global.user.desc,
